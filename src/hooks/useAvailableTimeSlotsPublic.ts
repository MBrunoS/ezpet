...
import { usePetShopProfileByUserId } from '@/hooks/queries/usePetShopProfileQuery';
import { useAppointmentsPublic } from '@/hooks/queries/useAppointmentsQuery';
import { useServicesPublic } from '@/hooks/queries/useServicesQuery';
import { Service, Appointment } from '@/types';

interface UseAvailableTimeSlotsPublicProps {
  userId: string;
  selectedDate: Date;
...
export function useAvailableTimeSlotsPublic({ 
  userId,
  selectedDate, 
  selectedService
}: UseAvailableTimeSlotsPublicProps) {
  const { data: profile } = usePetShopProfileByUserId(userId);
  const { data: appointments } = useAppointmentsPublic(userId);
  ...
  const hasTimeConflict = (startTime: Date, endTime: Date): boolean => {
    ...
    return dayAppointments.some((appointment: Appointment) => {
  ...
  const isCapacityReached = (time: Date): boolean => {
    ...
    const overlappingAppointments = dayAppointments.filter((appointment: Appointment) => {
      const appointmentStart = new Date(appointment.date);
      const service = services?.find(s => s.id === appointment.serviceId);
      const appointmentDuration = service?.duration || 60;
      const appointmentEnd = new Date(appointmentStart.getTime() + appointmentDuration * 60000);
